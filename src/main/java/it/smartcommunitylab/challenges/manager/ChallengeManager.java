...
//	@Scheduled(cron = '${scheduler.cron}')
	public void invokeGenerator() {
		...
		final ExecDate executionDate = env.getProperty("config.execDate") != null
                ? new ExecDate(env.getProperty("config.execDate")) : new ExecDate();
        logger.info("running on configuration {} and gamification engine url {}, execDate: {}",
        		env.getProperty("config.file"), env.getProperty("config.url"),
                executionDate.getInstantAsString());
        final Tasker tasker = new Tasker(!isEmptyString(env.getProperty("config.task"))? env.getProperty("config.task").split(","): new String[0]);
